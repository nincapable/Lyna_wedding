...
type GameRow = { code: string; stage: number; attempts: number; accept_any_code: boolean; updated_at: string };
...
export async function POST(request: NextRequest, context: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    const response = await db(`game_sessions?code=eq.${encodeURIComponent(game.code)}&stage=eq.2&updated_at=eq.${encodeURIComponent(game.updated_at)}`, {
      method: 'PATCH', body: JSON.stringify({ stage: accepted ? 3 : 2, attempts: game.attempts + 1, updated_at: new Date().toISOString() }),
    });
    ...
    const message = accepted ? 'Dragon reconnu ! L’engramme 2 est déverrouillé sur tous les appareils.'
    ...
    return NextResponse.json({ ...publicGame(updated), accepted, ...(accepted ? { unlockedStage: 2 } : {}), message });
